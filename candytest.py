    ...
    for y in range(TILESH):
        for x in range(TILESW - 2):
            ...
            if board[y][x] == 11 and board[y][x + 1] == 11 and board[y][x + 2] == 11:
                ...
                score += 50
                tile_11_count += 1
            # Normal matching tiles (This part is important for non-special tiles)
            elif(
...
def every_second():
    if not win_condition:
        ...
        check_gaps()
        add_new_tiles()
        # Check if the player has collected enough of each tile
        if (
            tile_9_count >= 20
            and tile_10_count >= 15
            and tile_11_count >= 10
        ):
            win_condition = True
            print("You Win!")
    else:
